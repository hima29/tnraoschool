...
require_once get_parent_theme_file_path( 'lib/envato_setup/envato-setup-tgm-plugin-activation-functions.php' );
...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		protected $theme_prefix = 'superwise_';
		...
		public function admin_menus() {
			...
			if ( $this->is_submenu_page() ) {
				...
				$add_subpage_function( $this->parent_slug, esc_html__( 'Setup Wizard', 'superwise' ), esc_html__( 'Setup Wizard', 'superwise' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			} else {
				add_theme_page( esc_html__( 'Setup Wizard', 'superwise' ), esc_html__( 'Setup Wizard', 'superwise' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			}
...
//			);
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text'      => esc_html__( '...verifying', 'superwise' ),
			) );
	...
	public function setup_wizard_header() {
		...
		<head>
			...
			echo '<t';
			echo 'itle>' . esc_html__( 'Theme &rsaquo; Setup Wizard', 'superwise' ) . '</ti' . 'tle>'; ?>
		...
		<a class="wc-return-to-dashboard"
		   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'superwise' ); ?></a>
		<!--		--><?php //endif; ?>
		</body>
		<?php
	...
	<?php
		...
		public function envato_setup_introduction() {
			...
			if ( false && isset( $_REQUEST['debug'] ) ) {
				...
				$exporter_path = get_template_directory() . '/lib/envato_setup/envato-setup-export.php';
				...
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'superwise' ), wp_get_theme() ); ?></h1>
				<p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.', 'superwise' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'superwise' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Not right now', 'superwise' ); ?></a>
				</p>
			<?php
			} else if ( get_option( 'envato_setup_complete', false ) ) {
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'superwise' ), wp_get_theme() ); ?></h1>
				<p><?php esc_html_e( 'It looks like you have already run the setup wizard. Below are some options: ', 'superwise' ); ?></p>
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again', 'superwise' ); ?></a>
					</li>
<!--					<li>-->
<!--						<form method="post">-->
<!--							<input type="hidden" name="reset-font-defaults" value="yes">-->
<!--							<input type="submit" class="button-primary button button-large button-next"-->
<!--							       value="--><?php //esc_attr_e( 'Reset font style and colors', 'superwise' ); ?><!--" name="save_step"/>-->
...
<!--					</li>-->
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Cancel', 'superwise' ); ?></a>
				</p>
			<?php
			} else {
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'superwise' ), wp_get_theme() ); ?></h1>
				<p><?php printf( esc_html__( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. It should only take 5 minutes.', 'superwise' ), wp_get_theme() ); ?></p>
				<p><?php esc_html_e( 'No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!', 'superwise' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'superwise' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
		...
		public function envato_setup_introduction_save() {
			...
			if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
				...
				?>
				<p>
					<strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.', 'superwise' ); ?></strong>
				</p>
				<?php
				return true;
			}
		...
		public function envato_setup_requirements() {
			...
			?>
			<h3><?php esc_html_e( 'Befeore proceeding, please watch installation video and read the requirements bellow', 'superwise' ); ?></h3>
			<p>
				<a style="margin-right: 20px;" target="_blank" href="https://www.youtube.com/watch?v=4NUL4OUaoGk"><img
						src="<?php echo get_template_directory_uri() . '/assets/img/theme-install-video.jpg' ?>"
						alt=""/></a>
			</p>
			<h2><?php esc_html_e( 'Important!', 'superwise' ); ?></h2>
			<h4><?php printf( esc_attr( 'To use %s Theme, you must be running:', 'superwise' ), $theme_name ); ?></h4>
			<ul style="list-style:circle;margin:10px 40px 13px;">
				<li><?php esc_html_e( 'WordPress 3.1 or higher', 'superwise' ); ?></li>
				<li><?php esc_html_e( 'PHP5.4 or higher', 'superwise' ); ?></li>
				<li><?php esc_html_e( 'and mysql 5 or higher', 'superwise' ); ?></li>
			</ul>
			<h4>
				<?php esc_html_e( 'Many issues that you may run into such as: white screen, demo content fails when importing and other similar issues are all related to low PHP configuration limits. The solution is to increase the PHP limits. You can do this on your own, or contact your web host and ask them to increase those limits to a minimum as follows:', 'superwise' ); ?>
			</h4>
			<ul style="list-style:circle;margin:10px 40px 13px;">
				<li><?php esc_html_e( 'max_execution_time 180', 'superwise' ); ?></li>
				<li><?php esc_html_e( 'memory_limit 128M', 'superwise' ); ?></li>
				<li><?php esc_html_e( 'post_max_size 32M', 'superwise' ); ?></li>
				<li><?php esc_html_e( 'upload_max_filesize 32M', 'superwise' ); ?></li>
			</ul>
			<h4><?php esc_html_e( 'We have tested it with Mac, Windows and Linux. Below is a list of items you should ensure your host can comply with:', 'superwise' ); ?></h4>
			<ul style="list-style:circle;margin:10px 40px 13px;">
				<li><?php esc_html_e( 'Check to ensure that your web host has the minimum requirements to run WordPress.', 'superwise' ); ?></li>
				<li><?php esc_html_e( 'Always make sure they are running the latest version of WordPress.', 'superwise' ); ?></li>
				<li><?php printf( esc_attr( 'You can download the latest release of WordPress from official %s website.', 'superwise' ), '<a href="https://wordpress.org/" target="_blank">WordPress</a>' ); ?></li>
				<li><?php esc_html_e( 'Always create secure passwords for FTP and Database.', 'superwise' ); ?></li>
			</ul>
			<?php esc_html_e( 'Visual Composer and Revolution Slider plugins are included with the theme but you do not get purchase codes for these plugins.', 'superwise' ); ?>
			</h4>
			<p>
				<?php esc_html_e( 'That means that automatic updates for the plugins will not be available to you unless you buy a regular license for the plugin. We do not recommend that you buy regular licenses for the plugins because they will be updated with theme updates.', 'superwise' ); ?>
			</p>
			<br/>
		<?php if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) : ?>
				<br/>
				<p>
					<a href="<?php echo admin_url( 'admin.php?page=wc-status' ); ?>"><?php esc_html_e( 'Check System Status page', 'superwise' ); ?></a>
				</p>
			<?php endif; ?>

			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button-primary button button-large button-next"><?php esc_html_e( 'Continue', 'superwise' ); ?></a>
			</p>
			<?php
		}
		...
		private function _get_plugins() {
			if ( function_exists( 'superwise_setup_wizard_get_plugins' ) ) {
				return superwise_setup_wizard_get_plugins();
			} else {
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php esc_html_e( 'Default Plugins', 'superwise' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:', 'superwise' ); ?></p>
				...
				<?php
					...
					echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.', 'superwise' ) . '</strong></p>';
				} ?>

				<p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress.', 'superwise' ); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_plugins"><?php esc_html_e( 'Continue', 'superwise' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'superwise' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
		<?php
		...
		public function ajax_plugins() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'superwise' ) ) );
			}
			$json = array();
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Activating Plugin', 'superwise' ),
					);
					break;
				}
			}
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Updating Plugin', 'superwise' ),
					);
					break;
				}
			}
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Installing Plugin', 'superwise' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'superwise' ) ) );
			}
			exit;
		...
		public function envato_setup_rev_slider() {
			...
			<form method="post" enctype="multipart/form-data">
				<p><?php esc_html_e( 'Please select Revolution slider to import', 'superwise' ); ?></p>

				<input type="file" name="import_file">

				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next"
					       value="<?php esc_attr_e( 'Continue', 'superwise' ); ?>" name="save_step"/>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'superwise' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function envato_setup_rev_slider_handler() {
			...
			<form method="post">
				<p><?php esc_html_e( 'Please select which sliders to import', 'superwise' ); ?></p>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_rev_slider"><?php esc_html_e( 'Continue', 'superwise' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'superwise' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function ajax_rev_slider_cleanup() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['cleanup'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'Error', 'superwise' ) ) );
			}
		...
		public function ajax_rev_slider() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slider'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slider Found', 'superwise' ) ) );
			}
			...
			if ( $json ) {
				...
				wp_send_json( array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'superwise' ),
				) );
			}
			exit;
		}
		...
		private function _content_default_get() {
			...
			foreach ( $available_content as $post_type => $post_data ) {
				if ( count( $post_data ) ) {
					...
					$content[ $post_type ] = array(
						...
						'description'      => sprintf( esc_html__( 'This will create default %s as seen in the demo.', 'superwise' ), $post_type_title ),
						'pending'          => esc_html__( 'Pending.', 'superwise' ),
						'installing'       => esc_html__( 'Installing.', 'superwise' ),
						'success'          => esc_html__( 'Success.', 'superwise' ),
			...
			$content['widgets']  = array(
				'title'            => esc_html__( 'Widgets', 'superwise' ),
				'description'      => esc_html__( 'Insert default sidebar widgets as seen in the demo.', 'superwise' ),
				'pending'          => esc_html__( 'Pending.', 'superwise' ),
				'installing'       => esc_html__( 'Installing Default Widgets.', 'superwise' ),
				'success'          => esc_html__( 'Success.', 'superwise' ),
			...
			$content['settings'] = array(
				...
				'description'      => esc_html__( 'Configure default settings.', 'superwise' ),
				'pending'          => esc_html__( 'Pending.', 'superwise' ),
				'installing'       => esc_html__( 'Installing Default Settings.', 'superwise' ),
				'success'          => esc_html__( 'Success.', 'superwise' ),
		...
		public function envato_setup_default_content() {
			?>
			<h1><?php esc_html_e( 'Default Content', 'superwise' ); ?></h1>
			<form method="post">
				<?php if ( $this->is_possible_upgrade() ) { ?>
					<p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'superwise' ); ?></p>
				<?php } else { ?>
					<p><?php printf( esc_html__( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ', 'superwise' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				<table class="envato-setup-pages" cellspacing="0">
					<thead>
					<tr>
						<td class="check"></td>
						<th class="item"><?php esc_html_e( 'Item', 'superwise' ); ?></th>
						<th class="description"><?php esc_html_e( 'Description', 'superwise' ); ?></th>
						<th class="status"><?php esc_html_e( 'Status', 'superwise' ); ?></th>
					</tr>
					</thead>
					<tbody>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_content"><?php esc_html_e( 'Continue', 'superwise' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'superwise' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function ajax_content() {
			...
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No content Found', 'superwise' ) ) );
			}
		...
		private function _fetch_remote_file( $url, $post ) {
			...
			if ( ! $upload || $upload['error'] ) {
				...
				if ( is_array( $response ) && ! empty( $response['body'] ) && $response['response']['code'] == '200' ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond', 'superwise' ) );
				}
				...
				if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size', 'superwise' ) );
				}

				if ( 0 == $filesize ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'superwise' ) );
				}

				if ( ! empty( $max_size ) && $filesize > $max_size ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s', 'superwise' ), size_format( $max_size ) ) );
				}
			}
		...
		public function _content_install_settings() {
			...
			update_option( SUPERWISE_THEME_OPTION_NAME, $redux_options );
			update_option( SUPERWISE_THEME_OPTION_NAME . '-transients', array( 'run_compiler' => 1 ) );
		...
		public function envato_setup_color_style() {
			...
			?>
			<h1><?php esc_html_e( 'Site Style', 'superwise' ); ?></h1>
			<form method="post">
				<p><?php esc_html_e( 'Please choose your site style below.', 'superwise' ); ?></p>
				...
				<div class="theme-presets">
					<ul>
						...
						foreach ( $this->site_styles as $style_name => $style_data ) {
							$class_name = $style_name == $current_style ? 'current' : '';
							?>
		...
		public function envato_setup_logo_design() {
			...
			?>
			<h1><?php esc_html_e( 'Logo', 'superwise' ); ?></h1>
			<form method="post">
				<p><?php printf( esc_html__( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard. Try %sEnvato Studio%s if you need a new logo designed.', 'superwise' ), '<a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">', '</a>' ); ?></p>
				...
				<table>
					<tr>
						...
						<td>
							<a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'superwise' ); ?></a>
						</td>
					</tr>
				</table>
		...
		public function envato_setup_updates() {
			?>
			<h1><?php esc_html_e( 'Theme Updates', 'superwise' ); ?></h1>
			<?php if ( function_exists( 'envato_market' ) ) { ?>
				<form method="post">
					...
					if ( count( $my_items ) ) {
						...
						<p class="envato-setup-actions step">
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							   class="button button-large button-next button-primary"><?php esc_html_e( 'Continue', 'superwise' ); ?></a>
						</p>
					<?php
					} else {
						?>
						<p><?php esc_html_e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'superwise' ); ?></p>
						<p>When an update becomes available it will show in the Dashboard with an option to install.</p>
						<p>
						...
						<p class="envato-setup-actions step">
							<input type="submit" class="button-primary button button-large button-next"
							       value="<?php esc_attr_e( 'Login with Envato', 'superwise' ); ?>" name="save_step"/>
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'superwise' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
		...
		public function envato_setup_customize() {
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button button-primary button-large button-next"><?php esc_html_e( 'Continue', 'superwise' ); ?></a>
			</p>
		...
		public function envato_setup_help_support() {
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue', 'superwise' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
		<?php
		}
		...
		public function envato_setup_ready() {
			...
			<h1><?php esc_html_e( 'Your Website is Ready!', 'superwise' ); ?></h1>
			...
			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php esc_html_e( 'Next Steps', 'superwise' ); ?></h2>
					<ul>
						<li class="setup-product"><a class="button button-primary button-large"
						                             href="https://twitter.com/aislinthemes"
						                             target="_blank"><?php esc_html_e( 'Follow @aislinthemes on Twitter', 'superwise' ); ?></a>
						</li>
						<li class="setup-product"><a class="button button-next button-large"
						                             href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'View your new website!', 'superwise' ); ?></a>
						</li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php esc_html_e( 'More Resources', 'superwise' ); ?></h2>
					<ul>
						<!--						<li class="documentation"><a href="http://dtbaker.net/envato/documentation/"-->
						<!--						                             target="_blank">-->
						<?php //esc_html_e( 'Read the Theme Documentation', 'superwise' ); ?><!--</a>-->
						<!--						</li>-->
						<li class="howto"><a href="https://wordpress.org/support/"
						                     target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'superwise' ); ?></a>
						</li>
						<li class="rating"><a href="http://themeforest.net/downloads"
						                      target="_blank"><?php esc_html_e( 'Leave an Item Rating', 'superwise' ); ?></a></li>
						<li class="support"><a href="https://themeforest.net/user/aislin"
						                       target="_blank"><?php esc_html_e( 'Get Help and Support', 'superwise' ); ?></a></li>
					</ul>
				</div>
			</div>
		<?php
		...
		public function envato_market_admin_init() {
			...
			if ( ! empty( $_POST['oauth_session'] ) && ! empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					$class         = 'error';
					echo "<div class=\"$class\"><p>" . sprintf( esc_html__( 'Something went wrong while trying to retrieve oauth token: %s', 'superwise' ), $error_message ) . '</p></div>';
				} else {
			...
			add_settings_section(
				...
				sprintf( esc_html__( 'Login for %s updates', 'superwise' ), $this->envato_username ),
			...
			add_settings_field(
				...
				esc_html__( 'oAuth Login', 'superwise' ),
				...
				if ( ! $option || empty( $option['oauth'] ) || empty( $option['oauth'][ $this->envato_username ] ) || empty( $envato_items ) || empty( $envato_items['items'] ) || ! envato_market()->items()->themes( 'purchased' ) ) {
					...
					if ( ! $dissmissed_time || $dissmissed_time < strtotime( '-7 days' ) ) {
						...
						<div class="notice notice-warning notice-dtbwp-themeupdates is-dismissible">
							<p><?php
								_e( 'Please activate ThemeForest updates to ensure you have the latest version of this theme.', 'superwise' );
								?></p>

							<p>
								<?php printf( __( '<a class="button button-primary" href="%s">Activate Updates</a>', 'superwise' ), esc_url( $this->get_oauth_login_url( admin_url( 'admin.php?page=' . envato_market()->get_slug() . '' ) ) ) ); ?>
							</p>
						</div>
