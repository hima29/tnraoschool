...
function mdocs_check_upload_defaults($type_formatted, $type_options) {
	...
	if(isset($atts[$type_formatted])) {
		if($type_formatted == 'folder') {
			$the_data = mdocs_get_the_folder($atts[$type]);
			if($the_data == null) $default_value = $cats[0]['slug'];
			else $default_value = $the_data['slug'];
		} else {
			$default_value = $atts[$type_formatted];
		}
	} elseif($type_options == 'folder') {
		$default_value = $cats[0]['slug'];
	} elseif($type_options == 'version') {
		$default_value = '1.0';
	} elseif($type_options == 'file-status') {
		$default_value = 'public';
	} elseif($type_options == 'post-status') {
		$default_value = 'publish';
	} elseif($type_options == 'show-social-apps') {
		$default_value = 'checked';
	} elseif($type_options == 'downloadable-by-non-members') {
		$default_value = 'checked';
	} elseif($type_options == 'contributors') {
		$default_value = '';
	} elseif($type_options == 'author') {
		$default_value = '';
	} else $default_value = '';
